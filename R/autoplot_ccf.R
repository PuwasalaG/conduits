...
autoplot.conditional_ccf <- function(object,
  ...
  if (type == "mean") {
    autoplot.conditional_moments(
      object = cond_moment_x,
      type = "mean"
    )
    autoplot.conditional_moments(
      object = cond_moment_y,
      type = "mean"
    )
  }
  ...
  if (type == "cross-correlation") {
    for (j in k) {
      ...
      for (i in 1:length(names_z_numeric)) {
        ...
        # predicted values
        predict_fit_ccf_link <- stats::predict.glm(ccf_gam_fit_k,
                                                   type = "link",
                                                   newdata = data_vis_z,
                                                   se.fit = T
        ) %>%
          ...
          dplyr::mutate(
            LI_link = as.numeric((.data$fit_link - .data$se.fit_link)),
        ...
        predict_fit_ccf_res <- stats::predict.glm(ccf_gam_fit_k,
          ...
          dplyr::mutate(
            LI_res = as.numeric((.data$fit_res - .data$se.fit_res)),
        ...
        data_vis_z <- list(
          data_vis_z, resid_ccf, predict_fit_ccf_link,
          predict_fit_ccf_res
        ) %>%
        ...
        plot_link[[i]] <- data_vis_z %>%
          ...
          geom_ribbon(aes(
            x = !!rlang::sym(name_zi), y = .data$fit_link,
        ...
        plot_response[[i]] <- ggplot() +
          ...
          geom_ribbon(
            data = data_vis_z,
            aes(
              x = !!rlang::sym(name_zi), y = .data$fit_res,
      ...
      gridExtra::grid.arrange(gridExtra::arrangeGrob(
        grobs = plot_link, ncol = n_col,
        top = "predictor scale"
      ),
      gridExtra::arrangeGrob(
        grobs = plot_response, ncol = n_col,
        top = "response scale"
      ),
      top = paste("k=", j, sep = "")
      )
...
autoplot.estimate_dt <- function(object,
  ...
  for (i in seq_along(names_z_numeric)) {
    ...
    z_i <- seq(
      from = min(z_i, na.rm = T),
      to = max(z_i, na.rm = T),
      length.out = 300
    )
    z_i_ <- data %>%
      dplyr::select(
        Timestamp, dplyr::all_of(names_z_numeric[-i]),
        {{ z_factors }}
      ) %>%
      dplyr::mutate_at(
        names_z_numeric[-i],
    ...
    new_data_bs <- tibble::tibble(
      Timestamp = data$Timestamp[1:300],
      z_i = z_i
    )
  ...
  for (i in 1:p) {
    data_vis_dt <- estimate_dt(
      object = object$conditional_ccf_object,
      new_data = list_visualise_data[[i]][[1]]
    )
    data_vis_dt <- data_vis_dt$data_dt %>%
      dplyr::select(
        .data$Timestamp, names_z_numeric[i], .data$dt,
        .data$ccf
      ) %>%
  ...
  if (interval == TRUE) {
    ...
    while (size_df < m) { # j = 13, 43
      ...
      bs_resid <- purrr::map_df(
        .x = arma_fit, .f = func_bootstrap_resid,
        size = N
      )
      ...
      for (l in seq_along(names_XY)) {
        ...
        GLM_out <- tryCatch(stats::glm(
          formula = stats::as.formula(formula_XY),
          data = data_ccf_gam,
    ...
    for (i in 1:p) {
      ...
      DF_ccf_bootstrap_CI <- DF_ccf_bootstrap %>%
        ...
        dplyr::group_by(.data$Timestamp) %>%
        dplyr::summarise(
          dt_95_LI = round(stats::quantile(.data$max_lag, probs = c(0.025))),
  ...
  for (i in 1:p) {
    if (interval == TRUE) {
      ...
      plot_list[[i]] <- list_visualise_data[[i]]$data_vis_dt %>%
        ...
        geom_point() +
        geom_ribbon(aes(
          ymin = .data$dt_95_LI, ymax = .data$dt_95_UI,
          group = 1
        ),
        alpha = 0.3, fill = "gray60"
        ) +
        geom_ribbon(aes(
          ymin = .data$dt_80_LI, ymax = .data$dt_80_UI,
          group = 1
        ),
